...
A stockbroker can give his client one of three recommendations regarding a certain stock: buy (B), hold (H), or sell (S). The stock can be one of three kinds: a winner (W ), mediocre (M ), or a loser (L). The stock-broker knows the type of stock but the client knows only that each type is equally likely. The game proceeds as follows: first, the stockbroker makes a recommendation $a_1 \in \{B, H, S\}$ to the client, after which the client chooses an action $a_2 \in \{B, H, S\}$ and payoffs are determined. The payoffs to the stock-broker (player 1) and client (player 2) depend on the type of stock and the action taken by the client (the pairs are $(v_1, v_2)$, where $v_i$ is player i's payoff) as follows:
